  ...
  try {
    ...
    switch (command) {
      ...
      case "upload": {
        ...
        const commandPath = join(global.mainPath, 'plugins', 'commands', `${commandName}.js`);
        if (!existsSync(commandPath)) return sendBeautifulMessage(`\n❌ 𝐅𝐢𝐥𝐞 '${commandName}.js' 𝐧𝐨𝐭 𝐟𝐨𝐮𝐧𝐝.`);
        try {
          const code = readFileSync(commandPath, 'utf8');
          let commandFile;
          try {
            commandFile = await import(commandPath);
          } catch (err) {
